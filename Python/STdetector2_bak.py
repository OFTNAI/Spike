...
experimentName = '1.6--FF_FB_LAT_stdp_0.005_I2Econst0.005_I2Ex10';
...
#     maxSCindex=np.zeros((nObj,nTrans,4,nExcitCells));
    ...
    spikePairDetector = np.zeros((nObj,nTrans,nLayers,spikePair_maxNum));#targetcell, 
...
#                     t_begin = max(presentationTime*(obj*nTrans+trans),spikeTimes_perStim[index_spike]-maxDelay);
                ...
                if spikeTimes_perStim[index_spike]>presentationTime*(obj*nTrans+trans)+maxDelay:
                    for t in range(t_begin,spikeTimes_perStim[index_spike]):
                        ...
                        if listOfCells_tmp.size>0:
                            probTable_list[obj,trans,targetSpikeID,listOfCells_tmp,t-t_begin]+=1.0;
            ...
            for targetSpikeID in range(exDim*exDim):
                if (spikeCountPerCell[targetSpikeID]<mimNumSpikeTh):#if the number of spikes of target cell was not satisfactory 
                    probTable_list[obj,trans,targetSpikeID] = 0;
                else:
                    probTable_list[obj,trans,targetSpikeID]/=spikeCountPerCell[targetSpikeID];
                    
                tmp = np.where(probTable_list[obj,trans,targetSpikeID]>SpikeChainDetectionProbTh);
...
#                         print(spikePairStr);
                    if spikePairStr not in SpikePair_string:
                        ...
                        spikePairDetector[obj,trans,l,SpikePair_string.index(spikePairStr)]=1;
...
#                 colorBarOn = False;
    ...
    nUniquePairs = len(SpikePair_string);
    nBins = 2;
    #infor analysis
    infos = np.zeros(nUniquePairs);
                
    sumPerBin = np.zeros((nUniquePairs,nBins));
    sumPerObj = nTrans;
    sumPerCell = nTrans*nObj;
    IRs = np.zeros((nObj,nUniquePairs));#I(R,s) single cell information
    IRs_weighted = np.zeros((nObj,nUniquePairs));#I(R,s) single cell information
    pq_r = np.zeros(nObj)#prob(s') temporally used in the decoing process
    Ps = 1/nObj   #Prob(s) 
    
    
    print("**Loading data**")
    binMatrix = np.zeros((nUniquePairs, nObj, nBins));# #number of times when fr is classified into a specific bin within a specific objs's transformations
    for obj in range(nObj):
        print str(obj) + '/' + str(nObj);
        for trans in range(nTrans):
            for cell in range(nUniquePairs):
#                             bin = np.around(FR_tmp[obj,trans,l,cell]*(nBins-1));
                bin = min(np.floor((spikePairDetector[obj,trans,l,cell])*(nBins)),nBins-1)
