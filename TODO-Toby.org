...
** TODO If prepare_backend hasn't been called before backend is needed, then we should not just crash. Instead, we should have a compile-time option to call prepare_backend implicitly using the global context. This would mean that adding prepare_backend calls to legacy code should not be needed.
