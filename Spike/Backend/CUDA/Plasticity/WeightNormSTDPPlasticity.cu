  ...
  namespace CUDA {
	...
	if (total_number_of_plastic_synapses > 0) {
		...
		for (int id = 0; id < total_number_of_plastic_synapses; id++){
			weight_values[id] = frontend()->syns->synaptic_efficacies_or_weights[frontend()->plastic_synapses[id]];
		}
		...
		CudaSafeCall(cudaMemcpy((void*)afferent_weight_change_updater,
				...
				sizeof(float)*frontend()->post_neuron_set.size(), cudaMemcpyHostToDevice));
    	}
    }
	...
	if (total_number_of_plastic_synapses > 0){
	  ...
	  CudaSafeCall(cudaMalloc((void **)&sum_squared_afferent_values, sizeof(float)*frontend()->post_neuron_set.size()));
	  CudaSafeCall(cudaMalloc((void **)&afferent_weight_change_updater, sizeof(float)*frontend()->post_neuron_set.size()));
	  CudaSafeCall(cudaMalloc((void **)&post_neuron_conversion, sizeof(float)*frontend()->post_neuron_set.size()));
	  // Copy values
	  ...
	  CudaSafeCall(cudaMalloc((void **)&weight_divisor, sizeof(float)*frontend()->post_neuron_set.size()));
	}
    }
		...
		size_t total_number_of_plastic_synapses)
	{
		...
		while (indx < total_number_of_plastic_synapses) {
			...
			int post_id = post_neuron_conversion[postsyn_ids[idx]];
    ...
    int* post_neuron_conversion,
		...
		while (indx < total_number_of_plastic_synapses) {
			int idx = d_plastic_synapse_indices[indx];
			int postneuron = post_neuron_conversion[postsyn_neuron[idx]];
