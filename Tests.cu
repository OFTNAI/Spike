...
TEST_CASE("Neuron Class Constructor", "[Neuron]") {
	...
	REQUIRE(neurons.total_number_of_neurons == 0);
	REQUIRE(neurons.total_number_of_groups == 0);
	REQUIRE(neurons.last_neuron_indices_for_each_group == NULL);
}
...
// 	REQUIRE(neurtest.total_number_of_neurons == 100);
// 	REQUIRE(neurtest.numPopulations == 1);
// 	REQUIRE(neurtest.numperPop[0] == neurtest.total_number_of_neurons);
// 	// Check that the values in every part of array
// 	for (int i = 0; i < neurtest.total_number_of_neurons; i++){
// 		// Set the parameters
...
// 	// Check that the number of neurons has been correctly set
// 	REQUIRE(neurtest.total_number_of_neurons == 128);
// 	REQUIRE(neurtest.numPopulations == 2);
// 	REQUIRE(neurtest.numperPop[0] == 100);
// 	REQUIRE(neurtest.numperPop[1] == neurtest.total_number_of_neurons);
// 	// Check that the values in every part of array
// 	for (int i = 0; i < neurtest.total_number_of_neurons; i++){
// 		if (i < 100){
// 			// Set the parameters
...
// 	REQUIRE(sim.population.total_number_of_neurons == 1000);
// 	REQUIRE(sim.population.numperPop[0] == 1000);
// 	// Check that the values in every part of array
// 	for (int i = 0; i < sim.population.total_number_of_neurons; i++){
...
TEST_CASE("Simulator Create Spike Generators", "[Simulator]"){
	...
	int output = sim.AddNeuronGroup(paramCort, rateShape);
	// Creating the Neurons that I wish to have in my generator population
	int lastlayer = sim.AddNeuronGroup(rate, rateShape);
	...
	REQUIRE(sim.neurons->total_number_of_neurons == 2000);
	// Run this network
	sim.Run(2.5f, 1, true);
...
// 	size_t total_number_of_neurons = 10;
...
// 	CudaSafeCall(cudaMalloc((void **)&d_randoms, sizeof(float)*total_number_of_neurons));
// 	CudaSafeCall(cudaMalloc((void **)&d_neuronpop_vars, sizeof(struct neuron_struct)*total_number_of_neurons));
// 	// Copying the data to the device
// 	CudaSafeCall(cudaMemcpy(d_randoms, h_randoms, sizeof(float)*total_number_of_neurons, cudaMemcpyHostToDevice));
// 	CudaSafeCall(cudaMemcpy(d_neuronpop_vars, h_neuronpop_vars, sizeof(struct neuron_struct)*total_number_of_neurons, cudaMemcpyHostToDevice));
...
// 	CudaSafeCall(cudaMemcpy(h_neuronpop_vars, d_neuronpop_vars, sizeof(struct neuron_struct)*total_number_of_neurons, cudaMemcpyDeviceToHost));
// 	// Free device memory
...
// 	CudaSafeCall(cudaMalloc((void **)&d_currinj, sizeof(float)*total_number_of_neurons));
// 	// Copying the data to the device
...
// 	CudaSafeCall(cudaMemcpy(d_currinj, h_currinj, sizeof(float)*total_number_of_neurons, cudaMemcpyHostToDevice));
// 	// Setting up dimensions
// 	dim3 threads(100,1,1);
// 	dim3 blocks(1,1,1);
// 	// Running the function
...
// 	CudaSafeCall(cudaMemcpy(e_currinj, d_currinj, sizeof(float)*total_number_of_neurons, cudaMemcpyDeviceToHost));
...
// 	CudaSafeCall(cudaMalloc((void **)&d_stdp, sizeof(int)*total_number_of_neurons));
// 	CudaSafeCall(cudaMalloc((void **)&d_postsyns, sizeof(int)*numConns));
// 	CudaSafeCall(cudaMalloc((void **)&d_weights, sizeof(float)*numConns));
// 	CudaSafeCall(cudaMalloc((void **)&d_lastactive, sizeof(float)*numConns));
// 	CudaSafeCall(cudaMalloc((void **)&d_lastspiketime, sizeof(float)*total_number_of_neurons));
// 	// Copying the data to the device
// 	CudaSafeCall(cudaMemcpy(d_stdp, h_stdp, sizeof(int)*total_number_of_neurons, cudaMemcpyHostToDevice));
// 	CudaSafeCall(cudaMemcpy(d_postsyns, h_postsyns, sizeof(int)*numConns, cudaMemcpyHostToDevice));
// 	CudaSafeCall(cudaMemcpy(d_weights, h_weights, sizeof(float)*numConns, cudaMemcpyHostToDevice));
// 	CudaSafeCall(cudaMemcpy(d_lastactive, h_lastactive, sizeof(float)*numConns, cudaMemcpyHostToDevice));
// 	CudaSafeCall(cudaMemcpy(d_lastspiketime, h_lastspiketime, sizeof(float)*total_number_of_neurons, cudaMemcpyHostToDevice));
// 	// Setting up dimensions
// 	dim3 threads(100,1,1);
// 	dim3 blocks(1,1,1);
// 	// Running the function
...
// 	CudaSafeCall(cudaMemcpy(e_neuronpop_vars, d_neuronpop_vars, sizeof(struct neuron_struct)*total_number_of_neurons, cudaMemcpyDeviceToHost));
...
// 	struct neuron_struct e_neuronpop_vars[10];
// 	CudaSafeCall(cudaMemcpy(e_lastspiketime, d_lastspiketime, sizeof(float)*total_number_of_neurons, cudaMemcpyDeviceToHost));
// 	CudaSafeCall(cudaMemcpy(e_neuronpop_vars, d_neuronpop_vars, sizeof(struct neuron_struct)*total_number_of_neurons, cudaMemcpyDeviceToHost));
// 	// Run the tests!
// 	for (int i = 0; i < total_number_of_neurons; i++){
// 		// Check for spiking
